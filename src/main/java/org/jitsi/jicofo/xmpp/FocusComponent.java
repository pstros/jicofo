...
public class FocusComponent extends ComponentBase implements BundleActivator {
    ...
    /**
     * Name of system and configuration property which specifies the JID from which
     * shutdown requests will be accepted.
     */
    ...
    /**
     * The JID of focus user that will enter the MUC room. Can be user to recognize
     * real focus of the conference.
     */
     ...
     * 
     * @param host           the hostname or IP address to which this component will
     *                       be connected.
     * @param port           the port of XMPP server to which this component will
     *                       connect.
     * @param domain         the name of main XMPP domain on which this component
     *                       will be served.
    ...
    public void start(BundleContext bc) throws Exception {
        ...
        String shutdownAllowedJid = configService.getString(SHUTDOWN_ALLOWED_JID_PNAME);
        this.shutdownAllowedJid = StringUtils.isNullOrEmpty(shutdownAllowedJid) ? null
     ...
     *
     * @param iq the <tt>org.xmpp.packet.IQ</tt> stanza of type <tt>set</tt> which
     *           represents the request to handle
     * @return an <tt>org.xmpp.packet.IQ</tt> stanza which represents the response
     *         to the specified request or <tt>null</tt> to reply with
    ...
    public IQ handleIQSetImpl(IQ iq) throws Exception {
        try {
            ...
            if (smackIq instanceof ConferenceIq) {
                ...
                if (shutdownAllowedJid == null || !shutdownAllowedJid.equals(bareFrom)) {
                    // Forbidden
                    XMPPError.Builder forbiddenError = XMPPError.getBuilder(XMPPError.Condition.forbidden);
    ...
    /**
     * Additional logic added for conference IQ processing like authentication and
     * room reservation.
     *
     * @param query      <tt>ConferenceIq</tt> query
     * @param response   <tt>ConferenceIq</tt> response which can be modified during
     *                   this processing.
    ...
    private org.jivesoftware.smack.packet.IQ handleConferenceIq(ConferenceIq query) throws Exception {
        ...
        if (authAuthority != null) {
            response.addProperty(new ConferenceIq.Property("externalAuth", String.valueOf(authAuthority.isExternal())));
        }
    ...
    private org.jivesoftware.smack.packet.IQ handleAuthUrlIq(LoginUrlIq authUrlIq) {
        ...
        if (StringUtils.isNullOrEmpty(machineUID)) {
            XMPPError.Builder error = XMPPError.from(XMPPError.Condition.bad_request,
     ...
     */
    private IQ handleColibriStatsIQ(ColibriStatsIQ iq) throws Exception {
        // Reply with stats
