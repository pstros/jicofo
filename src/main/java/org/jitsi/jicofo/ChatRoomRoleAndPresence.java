 ...
 * @author Pawel Domas
 */
public class ChatRoomRoleAndPresence implements ChatRoomMemberPresenceListener, ChatRoomMemberRoleListener,
        ChatRoomLocalUserRoleListener, AuthenticationListener {
    /**
    ...
    /**
     * The {@link JitsiMeetConferenceImpl} for which this instance is handling MUC
     * related stuff.
     */
     ...
     * join the room will become conference owner. When the owner leaves the room
     * next participant will be selected as new owner.
     */
    ...
    public ChatRoomRoleAndPresence(JitsiMeetConferenceImpl conference, ChatRoom chatRoom) {
    ...
    public void init() {
        ...
        authAuthority = ServiceUtils.getService(FocusBundleActivator.bundleContext, AuthenticationAuthority.class);
    ...
    /**
     * Disposes resources used and stops any future processing that might have been
    ...
    /**
     * Analyzes chat room events and simplifies them into 'member joined', 'member
     * left' and 'member kicked' events.
     *
     * {@inheritDoc}
     */
    @Override
    ...
    /**
     * Elects new owner if the previous one has left the conference. (Only if we do
     * not work with external authentication).
     */
    private void electNewOwner() {
        ...
        for (ChatRoomMember member : chatRoom.getMembers()) {
            if (conference.isFocusMember((XmppChatMember) member) || ((XmppChatMember) member).isRobot()
    ...
    public void localUserRoleChanged(ChatRoomLocalUserRoleChangeEvent evt) {
        if (logger.isDebugEnabled()) {
            logger.debug("Focus role: " + evt.getNewRole() + " init: " + evt.isInitial() + " room: "
    ...
    private boolean grantOwner(Jid jid) {
        try {
            ...
            logger.info("JM - chatroomrole granted ownership");
            return true;
    ...
    private void checkGrantOwnerToAuthUser(ChatRoomMember member) {
        ...
        if (ChatRoomMemberRole.OWNER.compareTo(member.getRole()) < 0) {
            ...
            if (authSessionId != null) {
                ...
                eventAdmin.postEvent(EventFactory.endpointAuthenticated(authSessionId, conference.getId(),
