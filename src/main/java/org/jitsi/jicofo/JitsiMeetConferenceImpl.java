...
public class JitsiMeetConferenceImpl
    ...
    /**
     * Jitsi Meet tool used for specific operations like adding presence extensions.
     */
    ...
    /**
     * A timeout task which will terminate media session of the user who is sitting
     * alone in the room for too long.
     */
    private Future<?> singleParticipantTout;

    /**
     * Contains the flags which indicate whether participants being invited to the
     * conference as a result of joining (as opposed to having already joined)
     * should be invited with the "start muted" option. The element at offset 0 is
     * for audio, at offset 1 for video.
     */
    private boolean[] startMuted = { false, false };

    /**
     * The name of shared Etherpad document. Is advertised through MUC Presence by
     * Jicofo user.
     */
    ...
    /**
     * <tt>ScheduledExecutorService</tt> service used to schedule delayed tasks by
     * this <tt>JitsiMeetConference</tt> instance.
     */
     ...
     * @param logLevel      (optional) the logging level to be used by this
     *                      instance. See {@link #logger} for more details.
     */
    public JitsiMeetConferenceImpl(EntityBareJid roomName, Resourcepart focusUserName,
            ProtocolProviderHandler protocolProviderHandler, ConferenceListener listener, JitsiMeetConfig config,
            JitsiMeetGlobalConfig globalConfig, Level logLevel, String id) {
     ...
     *                   considered broken in that case. Its stop method will be
     *                   called before throwing the exception to perform
     *                   deinitialization where possible.
     *                   {@link ConferenceListener}s will be notified that this
    ...
    public synchronized void start() throws Exception {
        ...
        logger.info("JM starting: " + getRoomName());
        try {
            ...
            if (jibriDetector != null && jibriOpSet != null) {
                logger.info("JM jibriDetector i shouldn't be here");
                jibriRecorder = new JibriRecorder(this, getXmppConnection(), executor, globalConfig);
            ...
            if (sipJibriDetector != null && jibriOpSet != null) {
                logger.info("JM - sip gateway i shouldn't be here");
                jibriSipGateway = new JibriSipGateway(this, getXmppConnection(),
            ...
            try {
                logger.info("JM - got an start execption", e);
                stop();
            } catch (Exception x) {
    ...
    /**
     * Stops the conference, disposes colibri channels and releases all resources
     * used by the focus.
     */
    ...
    private void joinTheRoom() throws Exception {
        ...
        chatRoom.join();
        logger.info("JM  - chatRoom joined()");
    ...
    private void leaveTheRoom() {
        ...
        logger.info("JM - leaveTheRoom");
        chatRoom.leave();
        logger.info("JM - leftRoom");
     ...
     *                       result of just having joined (as opposed to e.g.
     *                       another participant joining triggering the invite).
     */
     ...
     * @return the {@link Bridge}, or {@code null} if one could not be found or the
     *         participant already has an associated {@link Bridge}.
     */
    private Bridge selectBridge(Participant participant) {
        ...
        if (bridge == null) {
            bridge = bridgeSelector.selectBridge(this, participant);
        }
     ...
     * @param reInvite    whether the participant is to be re-invited or invited for
     *                    the first time.
     * @param startMuted  an array of size 2, which will determine whether the offer
     *                    sent to the participant should indicate that the
     *                    participant should start audio muted (depending on the
     *                    value of the element at index 0) and video muted
     *                    (depending on the value of the element at index 1).
     */
    ...
    /**
     * Re-calculates the Octo relays for bridges in the conference, and updates each
     * bridge session.
     */
     ...
     * @return the set of all Octo relays of bridges in the conference, except for
     *         {@code exclude}.
     */
    private List<String> getAllRelays(String exclude) {
        synchronized (bridges) {
            return bridges.stream().map(bridge -> bridge.bridge.getRelayId()).filter(Objects::nonNull)
     ...
     *              jitsi-videobridge instance for which to return the
     *              {@link BridgeSession}.
     */
     ...
     * @return the {@link BridgeSession} instance used by this
     *         {@link JitsiMeetConferenceImpl} which corresponds to a particular
     *         jitsi-videobridge instance represented by its JID, or {@code null} if
     *         the {@link Bridge} is not currently used in this conference.
    ...
    /**
     * Returns array of boolean values that indicates whether the last participant
     * have to start video or audio muted.
     * 
     * @param participant the participant
     * @param justJoined  indicates whether the participant joined the room now or
     *                    he was in the room before.
     * @return array of boolean values that indicates whether the last participant
     *         have to start video or audio muted. The first element should be
     *         associated with the audio and the second with video.
     */
    ...
    /**
     * Returns {@code true} if there are at least two non-focus participants in the
     * room.
     *
     ...
     *
     * @return <tt>true</tt> if given {@link ChatRoomMember} is a focus participant.
     */
    ...
    /**
     * Method called by {@link #rolesAndPresence} when one of the members has been
     * kicked out of the conference room.
     *
    ...
    /**
     * Method called by {@link #rolesAndPresence} when someone leave conference chat
     * room.
     *
    ...
    public XMPPError onSessionAccept(JingleSession jingleSession, List<ContentPacketExtension> answer) {
        ...
        try {
            Object[] sourcesAndGroupsAdded = tryAddSourcesToParticipant(participant, answer);
        ...
        logger.info("Received session-accept from " + participant.getEndpointId() + " with accepted sources:"
                + sourcesAdded);
     ...
     * @param sources      the <tt>MediaSourceMap</tt> with the sources to
     *                     advertise.
     * @param sourceGroups the <tt>MediaSourceGroupMap</tt> with source groups to
     *                     advertise.
     */
    private void propagateNewSources(Participant sourceOwner, MediaSourceMap sources,
        ...
        participants.stream().filter(otherParticipant -> otherParticipant != sourceOwner).forEach(participant -> {
            if (!participant.isSessionEstablished()) {
                logger.warn("No jingle session yet for " + participant.getEndpointId());
    ...
    /**
     * Adds the specified sources and source groups to the Octo participants of all
     * bridges except for {@code exclude}.
     * 
     * @param exclude      the bridge to which sources will not be added (i.e. the
     *                     bridge to which the participant whose sources we are
     *                     adding is connected).
    ...
    private void propagateNewSourcesToOcto(BridgeSession exclude, MediaSourceMap sources,
            MediaSourceGroupMap sourceGroups) {
        synchronized (bridges) {
            bridges.stream().filter(bridge -> !bridge.equals(exclude))
                    .forEach(bridge -> bridge.addSourcesToOcto(sources, sourceGroups));
        }
    }

    /**
     * Callback called when we receive 'transport-info' from conference participant.
     * The info is forwarded to the videobridge at this point.
     *
     * {@inheritDoc}
     */
    @Override
    ...
    public void onTransportReject(JingleSession jingleSession, JingleIQ reply) {
        ...
        if (p == null) {
            logger.error("No participant for " + Objects.toString(jingleSession));
            return;
        }
     ...
     * participant. New sources received are advertised to active participants. If
     * some participant does not have Jingle session established yet then those
     * sources are scheduled for future update.
     *
     * {@inheritDoc}
     */
    @Override
    ...
    /**
     * Callback called when we receive 'source-remove' notification from conference
     * participant. New sources received are advertised to active participants. If
     * some participant does not have Jingle session established yet then those
     * sources are scheduled for future update.
     *
    ...
    public XMPPError onRemoveSource(JingleSession sourceJingleSession, List<ContentPacketExtension> contents) {
        ...
        removeSources(sourceJingleSession, sourcesToRemove, sourceGroupsToRemove, true);
     ...
     *
     * @param sourceJingleSession source Jingle session from which sources are being
     *                            removed.
     ...
     */
    private void removeSources(JingleSession sourceJingleSession, MediaSourceMap sourcesToRemove,
        ...
        final MediaSourceMap removedSources = participant.removeSources(sourcesToRemove);

        final MediaSourceGroupMap removedGroups = participant.removeSourceGroups(sourceGroupsToRemove);
        ...
        synchronized (bridges) {
            bridges.stream().filter(bridge -> !bridge.equals(bridgeSession))
     ...
     *
     * @param participant - The {@link Participant} instance to which sources and
    ...
    private Object[] tryAddSourcesToParticipant(Participant participant, List<ContentPacketExtension> contents)
        ...
        MediaSourceMap newSources = MediaSourceMap.getSourcesFromContent(contents);
        MediaSourceGroupMap newGroups = MediaSourceGroupMap.getSourceGroupsForContents(contents);
     ...
     * state.
     *
     * @param except optional <tt>Participant</tt> instance whose source groups will
     ...
     * state.
     *
     * @param except a list of participants whose sources will not be included in
     *               the result.
     *
     * @return the list of all source groups of given media type that exist in
     *         current conference state.
     ...
     *
     * @return instance of <tt>JitsiMeetGlobalConfig</tt> used by this conference.
     */
    ...
    /**
     * Returns {@link System#currentTimeMillis()} timestamp indicating the time when
     * this conference has become idle(we can measure how long is it). -1 is
     * returned if this conference is considered active.
     *
     */
    ...
    boolean handleMuteRequest(Jid fromJid, Jid toBeMutedJid, boolean doMute) {
        ...
        if (principal == null) {
            logger.error("Failed to perform mute operation - " + fromJid + " not exists in the conference.");
            return false;
        }
        ...
        if (!fromJid.equals(toBeMutedJid)
            ...
            logger.error("Permission denied for mute operation from " + fromJid);
            return false;
        }
    ...
    /**
     * Returns current participants count. A participant is chat member who has some
     * videobridge and media state assigned(not just raw chat room member). For
     * example chat member which belongs to the focus never becomes a participant.
     */
    ...
    /**
     * Handles on bridge down event by shutting down the conference if it's the one
     * we're using here.
     */
    void onBridgeDown(Jid bridgeJid) {
        synchronized (bridges) {
            ...
            if (bridgeSession != null) {
                ...
                List<Participant> participantsToReinvite = bridgeSession.terminateAll();
    ...
    /**
     * Method called by {@link AbstractChannelAllocator} when it fails to allocate
     * channels with {@link OperationFailedException}. We need to make some
     * decisions here.
     *
     * @param channelAllocator the {@link AbstractChannelAllocator} instance which
     *                         is reporting the error.
     */
    void onChannelAllocationFailed(AbstractChannelAllocator channelAllocator, boolean retry) {
        ...
        if (channelAllocator instanceof ParticipantChannelAllocator) {
            onParticipantInviteFailed((ParticipantChannelAllocator) channelAllocator, retry, /* bridgeFailure */ true);
        } else if (channelAllocator instanceof OctoChannelAllocator) {
            onOctoChannelAllocationFailed((OctoChannelAllocator) channelAllocator);
        } else {
     ...
     */
    private void onOctoChannelAllocationFailed(OctoChannelAllocator channelAllocator) {
        logger.error("Failed to allocate Octo channels.");
        onBridgeDown(channelAllocator.getBridgeSession().bridge.getJid());
    }

    /**
     * A callback called by {@link ParticipantChannelAllocator} when establishing
     * the Jingle session with its participant fails.
     * 
    ...
    void onInviteFailed(ParticipantChannelAllocator channelAllocator) {
        onParticipantInviteFailed(channelAllocator, /* retry */ false, /* bridge failure */ false);
    }

    /**
     * Handles a failure to invite a participant to the conference. This could be
     * due to a failure to allocate colibri channels, or a Jingle failure.
     *
     * @param channelAllocator the {@link ParticipantChannelAllocator} which failed.
     ...
     *                         the jitsi-videobridge instance, in which case we
     *                         should consider the bridge unhealthy.
     */
    private void onParticipantInviteFailed(ParticipantChannelAllocator channelAllocator, boolean retry,
        ...
        if (retry) {
            inviteParticipant(participant, true, channelAllocator.getStartMuted());
        } else {
            terminateParticipant(participant, Reason.GENERAL_ERROR, "channel allocation or jingle session failed");
    ...
    /**
     * A method to be called by {@link AbstractChannelAllocator} just after it has
     * created a new Colibri conference on the JVB.
     * 
    ...
    /**
     * Creates the shared document name by either using the conference room name or
     * a random string, depending on configuration.
     *
     ...
     * <tt>Participant</tt> in the room to terminate its media session. There is no
     * point in streaming media to the videobridge and using the bandwidth when
     * nobody is receiving it.
     */
    ...
    class BridgeSession {
        ...
        /**
         * Indicates if the bridge used in this conference is faulty. We use this flag
         * to skip channel expiration step when the conference is being disposed of.
         */
         ...
         * 
         * @param bridge the {@link Bridge} which the new {@link BridgeSession} instance
         *               is to represent.
         */
        ...
        /**
         * Disposes of this {@link BridgeSession}, attempting to expire the COLIBRI
         * conference.
         */
        ...
        /**
         * Expires the COLIBRI channels (via {@link #terminate(Participant)}) for all
         * participants.
         * 
        ...
        /**
         * Expires the COLIBRI channels allocated for a specific {@link Participant} and
         * removes the participant from {@link #participants}.
         * 
         * @param participant the {@link Participant} for which to expire the COLIBRI
         *                    channels.
         * @return {@code true} if the participant was a member of {@link #participants}
         *         and was removed as a result of this call, and {@code false}
         *         otherwise.
         */
         ...
         * {@link Participant} in this {@link BridgeSession}, setting the participant's
         * RTP description, sources, transport information, etc.
         * 
         * @param participant
         */
        ...
        /**
         * Sends a COLIBRI message which updates the channels for the Octo participant
         * in this {@link BridgeSession}.
         */
        ...
        /**
         * Returns the Octo participant for this {@link BridgeSession}. If a participant
        ...
        /**
         * Returns the Octo participant for this {@link BridgeSession}. If a participant
         * doesn't exist yet, it is created and initialized with {@code relays} as the
         * list of remote Octo relays.
         * 
         * @return the {@link OctoParticipant} for this {@link BridgeSession}.
         */
         ...
         * participant. If the Octo participant's session is already established, then
         * the sources are added and a colibri message is sent to the bridge. Otherwise,
         * they are scheduled to be added once the session is established.
         * 
         * @param sources      the sources to add.
         * @param sourceGroups the source groups to add.
         */
         ...
         * 
         * @param allRelays all relays in the conference (including the relay of the
         *                  bridge of this {@link BridgeSession}).
         */
        ...
        /**
         * Creates an {@link OctoParticipant} for this {@link BridgeSession} and starts
         * an {@link OctoChannelAllocator} to allocate channels for it.
         * 
         * @param relays the list of Octo relay ids to set to the newly allocated
         *               channels.
    ...
    public String toString() {
        return "[JitsiMeetConferenceImpl, name=" + getRoomName().toString() + "]";
    }
}
