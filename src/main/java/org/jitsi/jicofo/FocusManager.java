 ...
 * @author Boris Grozev
 */
public class FocusManager implements JitsiMeetConferenceImpl.ConferenceListener, RegistrationStateChangeListener {
    /**
     ...
     */
    public static final String IDLE_TIMEOUT_PNAME = "org.jitsi.focus.IDLE_TIMEOUT";

    /**
     * Default amount of time for which the focus is being kept alive in idle mode
    ...
    /**
     * The name of configuration property that specifies XMPP domain that hosts the
     * conference and will be used in components auto-discovery. This is the domain
     * on which the jitsi-videobridge runs.
     */
    public static final String XMPP_DOMAIN_PNAME = "org.jitsi.jicofo.XMPP_DOMAIN";

    /**
     * The name of the configuration property that specifies XMPP domain of the
     * focus user.
     */
    public static final String FOCUS_USER_DOMAIN_PNAME = "org.jitsi.jicofo.FOCUS_USER_DOMAIN";

    /**
     * The name of configuration property that specifies the user name used by the
     * focus to login to XMPP server.
     */
    public static final String FOCUS_USER_NAME_PNAME = "org.jitsi.jicofo.FOCUS_USER_NAME";

    /**
     * The name of the configuration property that specifies login password of the
    ...
    /**
     * The name of the configuration property that specifies if certificates of the
     * XMPP domain name should be verified, or always trusted. If not provided then
     * 'false' (should verify) is used.
     */
    public static final String ALWAYS_TRUST_PNAME = "org.jitsi.jicofo.ALWAYS_TRUST_MODE_ENABLED";

    /**
     * The name of the property used to configure a 1-byte identifier of this Jicofo
     * instance, used for the purpose of generating conference IDs unique across a
     * set of Jicofo instances.
     */
    public static final String JICOFO_SHORT_ID_PNAME = "org.jitsi.jicofo.SHORT_ID";

    /**
     * The name of the configuration property used to configure the PubSub node to
     * which videobridges are publishing their stats. It is used to discover bridges
     * automatically.
     */
    public static final String SHARED_STATS_PUBSUB_NODE_PNAME = "org.jitsi.jicofo.STATS_PUBSUB_NODE";

    /**
     * A property to enable health check debug. Enabling this will result an extra
     * thread which will be monitoring the health-checks execution times. The thread
     * will print thread dump in the logs for those health checks which execute for
     * more than 3 seconds.
     */
    private static final String HEALTH_CHECK_DEBUG_PROP_NAME = "org.jitsi.jicofo.rest.HEALTH_CHECK_DEBUG_ENABLED";
     ...
     * {@link Map#size()} on it, which wouldn't be safe with a {@link HashMap} (as
     * opposed to a {@link ConcurrentHashMap}. I've chosen this solution, because I
     * don't know whether the cleaner solution of synchronizing on
     * {@link #conferencesSyncRoot} in {@link #getConferenceCount()} is safe.
     */
    ...
    /**
     * FIXME: remove eventually if not used anymore The list of
     * {@link FocusAllocationListener}.
     */
    ...
    /**
     * Observes and discovers JVB instances and other conference components on our
     * XMPP domain.
     */
    private ComponentsDiscovery componentsDiscovery;

    /**
     * Indicates if graceful shutdown mode has been enabled and no new conference
     * request will be accepted.
     */
    private boolean shutdownInProgress;

    /**
     * Handler that takes care of pre-processing various Jitsi Meet extensions IQs
     * sent from conference participants to the focus.
     */
    ...
    public void start() throws Exception {
        ...
        DomainBareJid xmppDomain = JidCreate.domainBareFrom(config.getString(XMPP_DOMAIN_PNAME));

        focusUserDomain = JidCreate.domainBareFrom(config.getString(FOCUS_USER_DOMAIN_PNAME));
        focusUserName = Resourcepart.from(config.getString(FOCUS_USER_NAME_PNAME));
        ...
        jitsiMeetServices = new JitsiMeetServices(protocolProviderHandler, focusUserDomain);
        ...
        healthChecksDebugEnabled = config.getBoolean(HEALTH_CHECK_DEBUG_PROP_NAME, false);
    }
     ...
     * @return <tt>true</tt> if conference focus is in the room and ready to handle
     *         session participants.
     * @throws Exception if for any reason we have failed to create the conference
     */
    public boolean conferenceRequest(EntityBareJid room, Map<String, String> properties) throws Exception {
        return conferenceRequest(room, properties, null /*
                                                         * logging level - not specified which means that the one from
                                                         * the logging configuration will be used
                                                         */);
    }
     ...
     * @throws Exception if for any reason we have failed to create the conference
     */
    public boolean conferenceRequest(EntityBareJid room, Map<String, String> properties, Level loggingLevel)
            throws Exception {
        if (room == null)
            return false;
        ...
        synchronized (conferencesSyncRoot) {
            ...
            if (isConferenceCreator) {
                ...
                logger.info("JM - is ConferenceCreator createCOnference: " + room + " propertie size: "
                        + properties.size());
        ...
        try {
            if (isConferenceCreator) {
                logger.info("JM - starting conference " + room);
                conference.start();
            }
        } catch (Exception e) {
     ...
     * @param properties configuration properties, see {@link JitsiMeetConfig} for
     ...
     */
    private JitsiMeetConferenceImpl createConference(EntityBareJid room, Map<String, String> properties, Level logLevel)
            throws Exception {
        ...
        JitsiMeetConferenceImpl conference = new JitsiMeetConferenceImpl(room, focusUserName, protocolProviderHandler,
                this, config, globalConfig, logLevel, id);

        logger.info("JM - puting the room into the hash: " + room);
        conferences.put(room, conference);
        conferenceIds.add(id);

        // JM - Should only do this if we need?
        ...
        if (conference.getLogger().isInfoEnabled())
            logger.info("Created new focus for " + room + "@" + focusUserDomain + " conferences count: "
                    + conferences.size() + " options:" + options.toString());

        // Send focus created event
        EventAdmin eventAdmin = FocusBundleActivator.getEventAdmin();
        if (eventAdmin != null) {
            eventAdmin.postEvent(EventFactory.focusCreated(conference.getId(), conference.getRoomName()));
        }
    ...
    private String generateConferenceId() {
        ...
        String jicofoShortId = FocusBundleActivator.getConfigService().getString(JICOFO_SHORT_ID_PNAME, "ff");

        String id;

        synchronized (conferencesSyncRoot) {
            do {
                id = jicofoShortId + Integer.toHexString(RANDOM.nextInt(0x1_0000));
     ...
     * @param reason   optional reason string that will be advertised to the users
     *                 upon exit.
     */
    ...
    public void conferenceEnded(JitsiMeetConferenceImpl conference) {
        ...
        synchronized (conferencesSyncRoot) {
            ...
            if (conference.getLogger().isInfoEnabled())
                logger.info("Disposed conference for room: " + roomName + " conference count: " + conferences.size());
            ...
            EventAdmin eventAdmin = FocusBundleActivator.getEventAdmin();
            if (eventAdmin != null) {
                eventAdmin.postEvent(EventFactory.focusDestroyed(conference.getId(), conference.getRoomName()));
            }
     ...
     * @return the {@code JitsiMeetConference} for the specified {@code roomName} or
     *         {@code null} if no conference has been allocated yet
     */
    ...
    /**
     * Enables shutdown mode which means that no new focus instances will be
     * allocated. After conference count drops to zero the process will exit.
     */
    ...
    private void maybeDoShutdown() {
        synchronized (conferencesSyncRoot) {
            if (shutdownInProgress && conferences.isEmpty()) {
                ...
                // multiple times.
                ShutdownService shutdownService = ServiceUtils.getService(FocusBundleActivator.bundleContext,
    ...
    /**
     * Returns <tt>true</tt> if graceful shutdown mode has been enabled and the
     * process is going to be finished once conference count drops to zero.
     */
    ...
    public interface FocusAllocationListener {
         ...
         * 
         * @param roomName the name of the conference room for which focus has been
         *                 destroyed.
         */
     ...
     * @return operation set instance of given class or <tt>null</tt> if given
     *         operation set is not implemented by focus XMPP provider.
     */
    ...
    private class FocusExpireThread {
        ...
        public FocusExpireThread() {
            timeout = FocusBundleActivator.getConfigService().getLong(IDLE_TIMEOUT_PNAME, DEFAULT_IDLE_TIMEOUT);
        }
